...
# ---- 3b) Attach Aridity Index (AI) per site and filter annual to AI < 0.65 ----
library(raster)
library(dplyr)
library(rlang)

# --- Detect latitude / longitude columns in `metadata` ---
name_lat <- grepl("(^|_)(lat|latitude)$", names(metadata), ignore.case = TRUE)
name_lon <- grepl("(^|_)(lon|long|lng|longitude)$", names(metadata), ignore.case = TRUE)

lat_col <- names(metadata)[name_lat]
lon_col <- names(metadata)[name_lon]

# Fallback: infer from numeric ranges if names weren't found
if (length(lat_col) == 0 || length(lon_col) == 0) {
  numdf <- dplyr::select(metadata, where(is.numeric))
  if (ncol(numdf) == 0) stop("No numeric columns in metadata to infer lat/lon from.")
  
  lat_score <- vapply(numdf, function(x) mean(!is.na(x) & x >= -90 & x <= 90), numeric(1))
  lon_score <- vapply(numdf, function(x) mean(!is.na(x) & x >= -180 & x <= 180), numeric(1))
  
  # pick best candidates
  lat_col <- if (length(lat_col)) lat_col[1] else names(sort(lat_score, decreasing = TRUE))[1]
  lon_candidates <- names(sort(lon_score, decreasing = TRUE))
  # prefer a lon column that actually extends beyond 90 degrees somewhere
  lon_col <- if (length(lon_col)) lon_col[1] else {
    cand <- lon_candidates[
      vapply(numdf[lon_candidates], function(x) any(abs(x) > 90, na.rm = TRUE), logical(1))
    ]
    if (length(cand) == 0) lon_candidates[1] else cand[1]
  }
}

# Build per-site coords (only for sites present in `annual`)
meta_coords <- metadata %>%
  dplyr::rename(latitude = !!sym(lat_col), longitude = !!sym(lon_col)) %>%
...
if (nrow(meta_coords) == 0) stop("No valid site coordinates after detection/cleaning.")

# Load AI raster (reproject if needed)
ai_r <- raster("~/Documents/RProjects/data/AI_4km.tif")
if (!raster::isLonLat(ai_r)) {
  ai_r <- raster::projectRaster(ai_r, crs = sp::CRS("+proj=longlat +datum=WGS84"))
}

# Extract AI values at site locations
meta_ai <- meta_coords %>%
  dplyr::mutate(
...
# Join to annual and filter to AI < 0.65
annual_ai_lt_065 <- annual %>%
  dplyr::left_join(dplyr::select(meta_ai, site, AI, Aridity_Class), by = "site") %>%
  dplyr::filter(!is.na(AI), AI < 0.65)

# If you want the rest of your workflow to use the filtered data:
annual <- annual_ai_lt_065

# Quick sanity checks (optional)
# message("Guessed latitude column: ", lat_col)
# message("Guessed longitude column: ", lon_col)
# table(meta_ai$Aridity_Class, useNA = "ifany")
# summary(annual_ai_lt_065$AI)

# (Optional) If you want the rest of your workflow to use this directly:
# annual <- annual_ai_lt_065
