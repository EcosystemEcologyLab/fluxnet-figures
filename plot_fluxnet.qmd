...
# amf_download_fluxnet is vectorized, so this could be done much more simply,
# but maybe the for loop is beneficial for error handling?
...
# map(zip_paths, function(zip_path) {
#   unzipped_path <- fs::path_ext_remove(zip_path)
#   unzip(zip_path, exdir = unzipped_path)
# })
...
# read_csv(daily_sites_paths) #this doesn't work because the CSVs have different columns
...
#   filter(if_any(any_of(cols_to_check), function(x) x < -9000))

# multiple_sites_annual <- multiple_sites_annual %>%
#   mutate(replace(
#     across(all_of(cols_to_check)),
#     across(cols_to_check) < -9000,
#     NA
#   ))

# Shouldn't we just replace all numeric values < -9000 with NA?
multiple_sites_annual <- multiple_sites_annual |>
...
mutiple_sites_daily <- multiple_sites_daily |>
  mutate(across(where(is.numeric), function(x) if_else(x < -9000, NA, x)))
```
...
ggplot(
  total_ts_ms,
  aes(x = date_object, y = GPP_NT_VUT_REF, color = site, fill = site)
) +
...
ggplot(
  total_ts_ms,
  aes(x = date_object, y = RECO_NT_VUT_REF, color = site, fill = site)
) +
  geom_point(size = 0.1, alpha = 0.1, key_glyph = draw_key_rect) +
  geom_vline(xintercept = yr_start_dates_ms, color = "grey", alpha = 0.5) +
  geom_hline(yintercept = 0, color = "grey", alpha = 0.5, linetype = "dashed") +
...
ggplot(
  total_ts_ms,
  aes(x = date_object, y = NEE_VUT_REF, color = site, color = fill)
) +
  geom_point(size = 0.1, alpha = 0.1, key_glyph = draw_key_rect) +
  geom_vline(xintercept = yr_start_dates_ms, color = "grey", alpha = 0.5) +
  geom_hline(yintercept = 0, color = "grey", alpha = 0.5, linetype = "dashed") +
...
ggplot(
  ...
  aes(x = TA_F, y = RECO_NT_VUT_REF, color = site)
) +
  geom_point() +
...
ggplot() +
  geom_polygon(
    data = Whittaker_biomes,
    aes(x = temp_c, y = precp_cm, color = biome),
    fill = "white"
  ) +
...
rad_dt <- single_site_hourly %>%
  mutate(
    date_object = ymd_hm(TIMESTAMP_START),
