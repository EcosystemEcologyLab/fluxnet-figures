...
```{r}
total_ts_ms_gpp <- multiple_sites_daily %>% 
  mutate(date_object = ymd(TIMESTAMP)) %>% 
...
yr_start_dates_ms <- total_ts_ms_gpp %>% 
  select(date_object) %>% 
  filter(grepl("-01-01", date_object)) %>% 
  pull()
```

Show entire time series of 

```{r}
ggplot(total_ts_ms_gpp, aes(x = date_object, y = GPP_NT_VUT_REF, color = site)) + 
...
ggplot(total_ts_ms_gpp, aes(x = date_object, y = running_mean_gpp, color = site)) +
