...
- Get list of AZ sites - contains all the sites in the list from Dave
- Do they have fluxnet available? It returns `Cannot find data from US-MtB` if not
- Add code to not redownload if it has already been downloaded

```{r}
az_sites <- site %>% 
  filter(STATE == "AZ")

all_site_dirs <- list.dirs("data")

for(site in az_sites$SITE_ID[16]){
  print(site)
  #site_dir_name <- paste0("data/", "AMF_", site, "_FLUXNET_FULLSET")
  #print(site_dir_name)
  if(site %in% all_site_dirs){
    print("Already downloaded")
  }
  #if(!dir.exists(site_dir_name))
  #if vector of dir names contains site name...
  #check to see if data folder for site already exists; if else
  #download
  #proceed on error? 
  # tryCatch({
  #   amf_download_fluxnet(user_id = "kristinariemer", 
