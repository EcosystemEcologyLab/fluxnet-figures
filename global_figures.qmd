...
> F1. FLUXNET site map by biome or record length. (Sec.2)
> F2. Network-mean annual fluxes (NEE, GPP, RECO, ET) with trends. (Sec.3)
> F3. Composite anomalies during extreme events (e.g., ENSO, drought). (Sec.3)
> F4. Global flux maps (baseline and anomaly layers). (Sec.4)
> F5. Regional highlight panels / bar charts by continent. (Sec.4)
> F6. Timeseries and normalized trends (Sec.5)
> F7. Phenology trends for major biomes (onset/peak/offset). (Sec.5)
> F8. “Year in Flux” thematic figure (rotating). (Sec.6)
