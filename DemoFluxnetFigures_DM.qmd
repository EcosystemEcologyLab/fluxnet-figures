...
max_gapfilled_bad  <- 0.70           # >70% filled => bad
...
annual_data <-
  ...
  load_fluxnet_data() |>
  dplyr::mutate(dplyr::across(where(is.numeric), ~na_if(.x, -9999))) |>
  dplyr::mutate(year = as.integer(TIMESTAMP), .before = TIMESTAMP) |>
  # adjust join key to your actual column names
  dplyr::left_join(md_join, by = "site")

# Gate by percent gap-filled (expects helper to create `is_bad`)
annual_flagged <- annual_data |>
  flag_bad_gapfilled(
...
# keep the good stuff
annual_data <- annual_flagged |>
  dplyr::filter(!is_bad)

# user-facing message
cat(sprintf(
  "Using '%s' as the QC variable. Records with > %.0f%% gap-filled data are flagged BAD. Drop if QC missing: %s\n",
  qc_gate_var,
  max_gapfilled_bad * 100,
  ifelse(drop_if_qc_missing, "YES", "NO")
))
...
# There are more than 15 IGBP codes!
# my_15_colors <- c(
...
p_whittaker <- ggplot() +
  ...
  labs(x = "Temperature (°C)", y = latex2exp::TeX("Precipitation ($cm\\ yr^{-1}$)")) +
  theme_classic() +
