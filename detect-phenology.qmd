...
manifest <-
  ...
  ungroup()
# daily <- load_fluxnet_data(manifest) # Don't actually want to read in all sites at once due to memory issues
```
...
detect_phenology_integral_site <- function(
  ...
  smooths <- map(years, \(year) {
    start <- make_date(year) - days(30)
    end <- ceiling_date(make_date(year), "year") + days(29)

    
    # TODO possibly adjust DOY of padded years so the previous year's are -1, 
    # -2, etc. and the next year's are 366, 367, 368, etc. just for better 
    # plotting?
  ...
  out <- smooths %>%
    map(\(df) {
      ...
      df %>%
        summarize(
          SOS = DOY[which.max(smooth >= max_flux * 0.2)],
          POS = DOY[smooth == max_flux],
          EOS = DOY[which.max(smooth <= max_flux * 0.2 & DOY > POS)],
  ...
  if (isTRUE(keep_data)) {
    smooths_min <- smooths %>%
      map(\(df) {
        df %>% select(date, all_of(flux_var), DOY, DOY_aligned, smooth)
      })
...
library(bench)
set.seed(3267)
sample_sites <- sample(unique(daily$site), 8)
...
Yeah, that cuts the compute time reduces memory allocated quite a bit.
...
pheno_all %>% 
  filter(site == "US-Ha1") %>% 
  ggplot(aes(y = year)) +
  geom_pointrange(aes(x = POS, xmin = SOS, xmax = EOS, color = is.na(issues))) +
  labs(title = "US-Ha1", color = "OK", x = "Season range (DOY)")

pheno_all %>% 
  filter(site == "US-SRM") %>% 
  ggplot(aes(y = year)) +
  geom_pointrange(aes(x = POS, xmin = SOS, xmax = EOS, color = is.na(issues))) +
  labs(title = "US-SRM", color = "OK", x = "Season range (DOY)")
 
```

Both have a year with a flag.  Let's take a closer look.
...
Here we have some multi-modal seasonality, so it is unsurprising that this method didn't detect the season correctly. The first peak is at DOY 55 and is higher than the second peak.  The other problem is that the start of the season is in the previous year at DOY 336.  I think we should code buffer year DOYs differently so they don't overlap with the focal year.
...
pheno_all %>% 
  ...
  filter(!is.na(issues)) %>%
  unnest(data) %>% 
  mutate(DOY_adj = case_when(
    year(date) == year - 1 ~ (date - make_date(year)) %>% as.numeric('days'),
    year(date) == year + 1 ~ DOY + 365 + leap_year(year),
    .default = DOY
  )) %>% pull(DOY_adj) %>% range()
```
...
pheno_all %>%
  filter(site == "US-Ha1") %>%
  unnest(data) %>%
  # remove the "padding" dates for easier plotting
  filter(year == year(date)) %>%
  ggplot(aes(x = DOY, y = smooth, color = year, group = year)) +
  geom_line() +
...
@panwar2023 mentions other methods besides using a 20% threshold.